...
@/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/thinking-models-execution.md

**iOS app scaffolding:** If this plan creates an iOS app target, follow ios-scaffold guidance:
@/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/ios-scaffold.md
...
@/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/executor-examples.md
</deviation_rules>
...
**See @/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/checkpoints.md**
...
**When the orchestrator passes both `MVP_MODE=true` and `TDD_MODE=true`:** Before running the implementation step of any task with `tdd="true"`, run the runtime gate from `@/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/execute-mvp-tdd.md`. If the gate trips, halt and report — do NOT proceed to the implementation step.
...
When running inside a Cursor worktree (`.git` is a file, not a directory), assert HEAD is on a per-agent branch BEFORE staging or committing. If HEAD has drifted onto a protected ref, HALT — never self-recover via `git update-ref refs/heads/<protected>`:
```bash
...
Use the Write tool to create files — never use `Shell(cat << 'EOF')` or heredoc commands for file creation.

**Use template:** @/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/templates/summary.md
