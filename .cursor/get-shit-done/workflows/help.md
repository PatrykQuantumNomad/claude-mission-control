...
- Generates `./.cursor/skills/spike-findings-[project]/` with references and sources
- Writes summary to `.planning/spikes/WRAP-UP-SUMMARY.md`
- Adds auto-load routing line to project .cursor/rules/
...
- Generates `./.cursor/skills/sketch-findings-[project]/` with design decisions, CSS patterns, HTML structures
...
- Saves timestamped note to `.planning/notes/` (or `/Users/patrykattc/work/git/claude-mission-control/.cursor/notes/` globally)
...
- **`/gsd-ultraplan-phase [phase]`** — [BETA] Offload plan phase to Cursor's ultraplan cloud; review in browser and import back.
