...
@/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/ui-brand.md
@/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/revision-loop.md
@/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/gate-prompts.md
...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/tdd.md` so gate enforcement rules are available during planning.
...
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `@/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/skeleton-template.md`.
...
Use conversational prompting with options:
...
**If found:**
1. Read template: `/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/templates/VALIDATION.md`
...
SPIKE_FINDINGS_PATH=$(ls ./.cursor/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1 || true)
SKETCH_FINDINGS_PATH=$(ls ./.cursor/skills/sketch-findings-*/SKILL.md 2>/dev/null | head -1 || true)
```
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/tdd.md to all eligible tasks:
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `@/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
...
${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Follow vertical-slice planning rules from @/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/planner-mvp-mode.md. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
</planning_context>
...
Use conversational prompting with these 3 options.
...
**If "Prioritize":** Use conversational prompting (multiSelect) to let user pick which items are "now" vs "later". Create CONTEXT.md for each sub-phase with the selected items.
...
Use conversational prompting for each gap (or batch if multiple gaps).
...
conversational prompting. Selecting "Proceed anyway" continues to step 13b but
...
if [ "$POST_PLANNING_GAPS" = "true" ]; then
  node "/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/bin/gsd-tools.cjs" gap-analysis --phase-dir "${PHASE_DIR}"
fi
```
