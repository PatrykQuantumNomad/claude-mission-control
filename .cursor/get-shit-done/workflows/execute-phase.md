...
- **Cursor:** Uses `Agent(subagent_type="gsd-executor", ...)` — blocks until complete, returns result
...
@/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/agent-contracts.md
@/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/context-budget.md
@/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/gates.md
</required_reading>
...
**Model resolution:** If `executor_model` is `"inherit"`, omit the `model=` parameter from all `Agent()` calls — do NOT pass `model="inherit"` to Agent. Omitting the `model=` parameter causes Cursor to inherit the current orchestrator model automatically. Only set `model=` when `executor_model` is an explicit model name (e.g., `"claude-sonnet-4-6"`, `"claude-opus-4-7"`).
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/planner-antipatterns.md
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/workflows/execute-plan.md` **inline**
...
between a large tool_result and the next assistant turn (seen on Cursor
...
3. **Spawn executor agents:**
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     # Cursor inherits the orchestrator model automatically.
     ...
     prompt="
       ...
       spawned by Cursor's `isolation="worktree"` use the `worktree-agent-<id>`
       ...
       prompt; silent bypass violates project .cursor/rules/ guidance (#2924).
       ...
       @/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/checkpoints.md
       @/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/tdd.md
       @/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/worktree-path-safety.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/references/executor-examples.md'}
       </execution_context>
...
4. **Wait for all agents in wave to complete.**
   ...
   **This fallback applies automatically to all runtimes.** Cursor's Agent() normally
...
5.5. **Worktree cleanup (when `isolation="worktree"` was used):**
   ...
   # `.claude/worktrees/agent-` (the namespace Cursor's `isolation="worktree"`
...
If `TEXT_MODE` is true, present as a plain-text numbered list. Otherwise use conversational prompting.
...
Read and follow `/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
