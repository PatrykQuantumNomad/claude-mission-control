...
> `/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/USER-PROFILE.md` exists). Skip the Read
...
```bash
PROFILE_CONTENT=$(cat "/Users/patrykattc/work/git/claude-mission-control/.cursor/get-shit-done/USER-PROFILE.md" 2>/dev/null || true)
```
...
2. For EACH user-selected gray area, spawn a `Agent()` in parallel:
   ...
   ```
   Agent(
     prompt="First, read @/Users/patrykattc/work/git/claude-mission-control/.cursor/agents/gsd-advisor-researcher.md for your role and instructions.
...
2. **Use conversational prompting** (or text-mode equivalent if `--text` overlay):
   - header: `{area_name}`
   - question: `Which approach for {area_name}?`
   - options: extract from the table's Option column (conversational prompting adds
     "Other" automatically)
...
5. **After recording pick, decide whether follow-up questions are needed:**
   - If the pick has ambiguity that would affect downstream planning →
     ask 1-2 targeted follow-up questions using conversational prompting
