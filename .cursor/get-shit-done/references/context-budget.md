...
Every enabled MCP server injects its tool schema into **every turn**, regardless of whether you call any of its tools. Heavyweight servers can cost 20k+ tokens per turn each — often dwarfing whatever GSD itself can save through `model_profile` tuning. This is a Cursor harness concern, not a GSD concern: GSD does **not** manage MCP enablement. The toggle lives in `.claude/settings.json` under `enabledMcpjsonServers` and `disabledMcpjsonServers`.
...
The keys live in `.claude/settings.json` (project) or `/Users/patrykattc/work/git/claude-mission-control/.cursor/settings.json` (global) — **not** in `.planning/config.json`:
