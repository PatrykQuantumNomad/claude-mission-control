...
status: "Phase 25 wave 2 partially closed — Plan 02 backend CRUD shipped. Backend pytest 665 → 686 / 0 / 0. 5 /api/views endpoints with 50-cap + UNIQUE rejection live. Plan 05 (frontend hook layer) is the remaining wave 2 deliverable."
last_updated: "2026-05-12T14:32:39.000Z"
last_activity: "2026-05-12 — Plan 02 shipped 3 atomic commits (2eb5f9f schemas + 363320c router+mount + 8f78107 21-spec router coverage). Backend pytest 665 → 686 / 0 / 0. 5 /api/views CRUD endpoints (list/create/get/patch/delete) under views_router; VIEW_CAP_PER_ROUTE=50 application-side enforcement; UNIQUE (route, name) IntegrityError→HTTP-400 translation; state_json round-trips OPAQUELY per VIEW-02; route NOT patchable (intrinsic to view identity). 2 Rule-1 fixes (rollback-expires-attrs error-message + 50-cap-is-per-route invariant spec) + 1 Rule-3 fix (plan's async_session→db_session fixture name). Wave-2 frontend (Plan 05) now unblocked — full curl matrix in 25-02-SUMMARY.md."
progress:
  total_phases: 5
  completed_phases: 1
  total_plans: 18
...
Phase: 25 — Saved Views (Backend + Frontend) (11 plans authored; wave 2 partially closed — Plan 02 backend CRUD shipped; Plan 05 remaining)
Plan: Phase 25 plan-02 ✅ complete (3/3 tasks) — 5 /api/views CRUD endpoints + Pydantic schemas + 21-spec pytest matrix
Status: Wave 2 partially closed 2026-05-12 — Plan 02 (backend POST/GET/PATCH/DELETE /api/views) shipped. Commits 2eb5f9f (schemas) + 363320c (router+mount) + 8f78107 (21-spec test coverage + Rule-1 rollback-expiry fix + async factory). Backend pytest 665 → 686 / 0 / 0. ROADMAP Phase 25 success criterion 5 ("5 CRUD endpoints pass independently via curl + pytest before frontend wires up") satisfied. Next: spawn Plan 05 (frontend useSavedViews hook layer) — depends on this plan's API surface AND Plan 03's validateSearch named exports.
Last activity: 2026-05-12 — Plan 02 shipped 3 atomic commits. 5 endpoints under /api/views (list/create/get/patch/delete) wired into all_routers() after alerts_router. VIEW_CAP_PER_ROUTE=50 application-side enforcement via count-then-insert (Pitfall 5 acceptable for single-user). UNIQUE (route, name) IntegrityError translates to HTTP 400 with informative {error: detail} envelope. state_json round-trips OPAQUELY per VIEW-02 (dict[str, Any]; no backend schema validation). PATCH state_json REPLACES wholesale (NOT deep merge — pinned by test). route NOT in SavedViewUpdate (intrinsic to view identity — pinned by test). DELETE → 204 + empty body. 2 Rule-1 fixes: (1) capture row.route BEFORE setattr loop in patch_view so the IntegrityError error message survives the rollback-induced attr expiry (SQLAlchemy 2.x expires session attrs on rollback regardless of expire_on_commit=False); (2) per-route cap invariant pinned by extra spec test_create_view_cap_is_per_route. 1 Rule-3 fix: plan referenced async_session fixture that does not exist; used existing db_session (conftest.py:396-422) which deliberately coexists with client by sharing seeded_app. 1 Rule-2 addition: test_patch_view_route_field_silently_ignored pins that PATCH ignores `route` key (would otherwise silently regress if SavedViewUpdate ever gains a route field).

Progress (Phase 25 plans): [████░░░░░░░] 36% (4/11 plans complete — Plans 01, 02, 03, 04)
...
Phase 25 wave 2 partially closed 2026-05-12 — Plan 02 (backend /api/views CRUD) shipped. Wave 1 (Plans 01 + 03 + 04) + Plan 02 = 4/11 plans complete. Backend pytest 665 → 686 / 0 / 0; frontend vitest 380/380 (unchanged, no frontend touched this plan). ROADMAP Phase 25 success criterion 5 SATISFIED ("5 CRUD endpoints pass independently via curl + pytest before frontend wires"). Spawn Plan 05 next (frontend useSavedViews hook layer); it depends on this plan's API surface AND Plan 03's validateSearch named exports. Locked invariants now established for the hook layer:
...
- **Plan 03/04 (frontend search schemas):** `validateSearch` is a named export from every route file (`import { validateSearch } from '../../routes/<name>'`) — Plan 05 hook can import and re-coerce a raw search blob on save. `schemaVersion?: typeof SCHEMA_VERSION` is optional on input, always populated on output. `frontend/src/lib/searchSchemas.ts` exports `SCHEMA_VERSION` + `coerceSchemaVersion` — migration seam for future version bumps.
