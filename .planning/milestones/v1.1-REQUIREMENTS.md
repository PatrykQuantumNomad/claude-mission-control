# Requirements Archive: v1.1 Skills & Cost Intelligence
...
- [x] **ANLY-05**: User can see "Rates as of YYYY-MM-DD" caption on every cost figure rendered in the UI; doctor.py warns when pricing rows are >30 days old or `unpriced_tokens > 0` (backend portion delivered Phase 13 Plan 04 freshness endpoint + Plan 05 doctor sensors; UI caption delivered Phase 14 panels)
...
- [x] **ALRT-01**: User can rely on a new `alert_rules` SQLModel table with structural columns (`rule_id`, `name`, `kind`, `metric`, `threshold_fire`, `threshold_clear`, `min_dwell_seconds`, `min_samples`, `cooldown_seconds`, `enabled`, `spec_version`) plus `params_json` overflow for kind-specific config (table live since Phase 13 migration 0002; cosmetic doc-drift `[ ]` flipped at archive time — exercised by 77 passing tests)
- [x] **ALRT-02**: User can rely on a new `alert_state` table tracking `(rule_id, scope_key)` lifecycle (`firing` / `clear` / `acked` / `insufficient_data`) for hysteresis and dedup (table live since Phase 13 migration 0002; cosmetic doc-drift `[ ]` flipped at archive time)
...
| ANLY-05 | Phase 13 | Complete (backend Plan 04 freshness + Plan 05 doctor; UI caption Phase 14 panels) |
...
| ALRT-01 | Phase 15 | Complete (table live since Phase 13 migration 0002; doc-drift `[ ]` marker flipped at archive) |
| ALRT-02 | Phase 15 | Complete (table live since Phase 13 migration 0002; doc-drift `[ ]` marker flipped at archive) |
...
| CMPR-04 | Phase 16 | Complete (Plan 16-01 backend + Plan 16-02 frontend + Plan 16-04 browser verify check #4 — 2026-05-05) |
| CMPR-05 | Phase 16 | Complete (Plan 16-02 source-side grep + Plan 16-04 runtime DevTools Sources scan — 2026-05-05) |
| POLI-01 | Phase 17 | Complete |
| POLI-02 | Phase 17 | Complete |
| POLI-03 | Phase 17 | Complete |
| POLI-04 | Phase 17 | Complete |
| POLI-05 | Phase 17 | Complete (in-repo docs only; companion guide externally maintained per PROJECT.md:94) |
...
**Shipped:** 41 of 41 requirements
**Adjusted:** 0
**Dropped:** 0

Notable scope notes:

- **CMPR-01 module path drift:** REQUIREMENTS.md cited `cmc/cost/engine.py`; actual module is `cmc.pricing` (Phase 13 Plan 01 decision). Code path was always source of truth; no shim file created. Phase 17 README docs cleanup updated the citation in user-facing docs.
- **POLI-05 scope clarification:** Closed via in-repo docs only (README.md + backend/.env.example). Companion guide `build-your-own-dashboard-guide.html` is externally maintained per PROJECT.md:94 — out of scope for this repo.
- **ALRT-01 / ALRT-02 doc-drift markers:** Tables live in the codebase since Phase 13 migration 0002 and exercised by 77 passing tests. Cosmetic `[ ]` markers in active REQUIREMENTS.md flipped at archive time.

---

*Archived: 2026-05-05 as part of v1.1 milestone completion*
*Originally defined: 2026-05-02 — traceability section populated by gsd-roadmapper (41/41 mapped)*
