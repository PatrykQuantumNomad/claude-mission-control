...
> All five open questions were resolved during `/gsd:discuss-phase` — see `13-CONTEXT.md` `<decisions>` for the locked outcomes. Each entry below is annotated with its resolution inline.

1. **O-1: Are `claude-opus-4-7[1m]` and `claude-sonnet-4-6[1m]` separate API rate SKUs, or aliases of the base model?**

   **(RESOLVED — option (b))** CONTEXT.md `<decisions>` § "Pricing SKU model" locks **5 rate rows in `data/pricing.json`**, with `[1m]` rows carrying rates identical to their base counterparts at the 2026-05-03 freeze date. Honors ANLY-02's literal enumeration and survives a future Anthropic split of long-context pricing without schema change.
...
2. **O-2: Is `event.sequence` monotonic across `claude --resume` within a single `session.id`?**
   ...
   **(RESOLVED — `(session_id, otel_event_id)` UNIQUE where `otel_event_id` = `event.sequence`)** CONTEXT.md `<decisions>` § "Migration scope" delegates to planner: planner inspected production data during planning (see 13-02-PLAN.md `<objective>` lines 58-63 — 9,365 of 9,367 rows have `event.sequence`; pair `(session.id, event.sequence)` is unique across 9,386 rows with zero collisions). NULL-tolerance is acceptable per Pitfall 4. If a future resumed-session collision surfaces, the doctor's `_check_session_id_null_count` regression detector flags it.
...
3. **O-3: Does Phase 13 cover the cache TTL split JSONL parser change, or is that out of scope?**
   ...
   **(RESOLVED — IN SCOPE)** CONTEXT.md `<decisions>` § "Cache TTL split scope" locks: **JSONL parser change lands in Phase 13** (Claude's discretion confirmed by the explicit backfill choice). Migration adds `tokens_cache_create_5m` + `tokens_cache_create_1h` columns; backfill rule for legacy rows: all aggregate `tokens_cache_create` lands in `tokens_cache_create_1h`, with `5m = 0` (pessimistic). Implemented via Plan 02 (schema) + Plan 03 (parser).
...
4. **O-4: Should the seed loader run automatically in `lifespan.py`, or only via an explicit `cmc seed pricing` CLI command?**
   ...
   **(RESOLVED — auto-seed on lifespan startup; CLI deferred)** CONTEXT.md `<decisions>` § "Pricing seed + freshness UX" locks: **Auto-seed on FastAPI lifespan startup**. Idempotent (parsed-hash check skips no-op writes). No CLI in Phase 13 — the `cmc seed pricing` CLI is captured in `<deferred>` as a v1.2 candidate. Implemented in Plan 01 Task 2.
...
5. **O-5: Frontend — does Phase 13 ship the "Rates as of …" caption, or is that Phase 14?**
   ...
   **(RESOLVED — Phase 13 ships the API contract only; Phase 14 ships the React caption)** CONTEXT.md `<decisions>` § "Pricing seed + freshness UX" locks the `rates_as_of` field on every cost endpoint payload + a dedicated `GET /api/pricing/freshness` endpoint, AND the `<domain>` boundary explicitly locks out "All UI rendering (panels, captions, badges) — Phase 14 owns the UI." Implemented in Plan 04.
