...
async def test_alembic_upgrade_creates_all_tables(test_settings):
    ...
    try:
        ...
        # 19 app tables + alembic_version: 15 from 0001_initial + 3 from
        # 0002 (pricing — Phase 13 Plan 01; alert_rules + alert_state — Plan 02)
        # + 1 from 0004 (saved_views — Phase 25 VIEW-02).
        # (Migration 0003 added a column to sessions but no new table.)
        app_tables = [t for t in tables if t != "alembic_version"]
        assert len(app_tables) == 19, (
            f"Expected 19 app tables, got {len(app_tables)}: {app_tables}"
        )
        assert "saved_views" in app_tables, (
            f"saved_views table missing after upgrade head: {app_tables!r}"
        )
    finally:
        await engine.dispose()
...
async def test_lifespan_creates_all_tables(test_settings):
    """FOUND-02 + FOUND-05: Lifespan runs alembic upgrade -> all 19 tables exist.

    19 = 15 (0001_initial) + 3 (0002: pricing, alert_rules, alert_state)
       + 1 (0004: saved_views — Phase 25 VIEW-02).
    Migration 0003 added a column to sessions but no new table.
    """
    from fastapi import FastAPI
    from sqlalchemy import inspect
    ...
    async with lifespan(app):
        ...
        app_tables = [t for t in tables if t != "alembic_version"]
        assert len(app_tables) == 19, f"got {app_tables}"
        assert "saved_views" in app_tables, f"saved_views missing: {app_tables!r}"
