"""Optional stateless JWT validation service."""
...
class JWTAuthService:
    """Validate externally issued JWTs against shared-secret, static key, or JWKS."""
    ...
    async def readiness_check(self, app: FastAPI) -> ReadinessCheckResult:
        ...
        if self._uses_shared_secret():
            return ReadinessCheckResult.ok("auth", detail="Shared-secret JWT configured")
    ...
    async def _resolve_key(self, token: str) -> Any:
        if self.settings.auth_jwks_url:
            ...
            kid = header.get("kid")
            if not kid:
                raise AuthenticationError("JWT is missing kid header")
            ...
            key = _get_jwk_key_for_kid(refreshed_jwks, kid)
            if key is not None:
                return key
        ...
        raise AuthenticationError("JWT validation enabled without key material")
    ...
    def _uses_shared_secret(self) -> bool:
        return bool(self.settings.auth_jwt_secret) and any(
            algorithm.startswith("HS") for algorithm in self.settings.auth_jwt_algorithms
        )
